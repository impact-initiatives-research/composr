...
```{r}
df %>% new_recoding(target = is_B, source = select_one_letters) %>% 
...
## Recoding complex conditions across source variables: "Free recoding""
...
Here you _only_ supply the input data, the "to" value you want to recode to and a generic `where` condition. The "where"" condition can be any R code that can run inside the namespace of the input data (hopefully clear to tidyverselers, and to everyone else with the example below):
...
df %>% new_recoding(target = free_recoded_index) %>% recode_to(to="A-G and smaller than fifteen",
            where = c(select_one_letters %in% letters[1:7]) & some_numbers < 15) %>% end_recoding()
...
df %>% new_recoding(target = severity) %>% 
  recode_to(5,
            where = sm_selected(select_multiple_letters, any = c("A C","B"))) %>% 
  end_recoding
  

```

<div style="color:red;">not implemented yet: handling skiplogic in recode_freely</div>
