...
As you may have noticed, the recoding tracks what conditions are applied when and how often each comes into effect (printed to the console when you print the resulting data frame.). You can get rid of that and retreive the original dataset (with the new variable added) in the end with `end_recoding()`.
